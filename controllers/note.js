...
router.post('/createNote', function (req,res){
  let { title, body, headlineId } = req.body;
  let note = {
    title,
    body
  }
  db.Note
    ...
    .then( result => {
      db.Headline
        .findOneAndUpdate({_id: headlineId}, {$push:{notes: result._id}},{new:true})//saving reference to note in corresponding article
...
router.post('/deleteNote', (req,res)=>{
  let {headlineId, noteId} = req.body
  db.Note
    .remove({_id: noteId})
    .then(result => res.json(result))
    .catch(err => res.json(err));
